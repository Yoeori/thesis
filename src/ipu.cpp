...
auto getIpuModel(const unsigned int numIpus = 1, const unsigned int tilesPerIpu = 10) -> optional<Device> {
...
auto createGraphAndAddCodelets(const Device &device) -> Graph {
    auto graph = poplar::Graph(device.getTarget());

    // Add our own codelets
    graph.addCodelets({ "codelets/MatrixBlock.cpp", "codelets/Reducer.cpp" }, "-O3 -I codelets");
